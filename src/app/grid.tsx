...
import AoGridResizer from './grid-resizer'
...
class GridView extends React.PureComponent<GridViewProps, GridViewState> {
  ...
  dropToGridSquare(move: CardPlay) {
    ...
    const cardTo = this.props.aoStore.hashMap.get(move.to.taskId)
    ...
    switch (move.from.zone) {
      ...
      case 'subTasks':
        ...
        break
      case 'discard':
        this.props.aoStore.popDiscardHistory()
      case 'completed':
      case 'context':
      case 'panel':
      default:
  ...
  render() {
    ...
    for (let j = 0; j < grid.height; j++) {
      for (let i = 0; i < grid.width; i++) {
        ...
        const card = this.props.aoStore.hashMap.get(tId)
        render.push(
          <AoDropZone
            taskId={tId}
            inId={taskId}
...
@observer
class AoGrid extends React.PureComponent<GridProps, GridState> {
  constructor(props) {
    super(props)
...
export default withUseStore(AoGrid)
