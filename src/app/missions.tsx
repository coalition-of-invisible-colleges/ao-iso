...
@observer
class AoMissions extends React.PureComponent<Props, State> {
  constructor(props) {
    super(props)
  ...
  get renderMissionsList() {
    let missions = this.props.aoStore.state.tasks.filter(task => {
    ...
    projectCards = projectCards.concat(
      ...missions.map(task => this.props.aoStore.subGuildsByGuild.get(task.taskId))
    )
...
export default withUseStore(AoMissions)